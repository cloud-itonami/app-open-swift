...
;; (test/openswift/route_test.cljc) はソースの判断を固定するが、bundle が
...
(def sentinel
  ...
  落ちなくなる。app-ongakuka が実測でこれを踏んだので、印を使う。"
  "SENTINEL-7b41d0")
...
(def env #js {"APP_HANDLE" "open-swift.etzhayyim.com"
              "PRIMARY_DID" sentinel
              "AGENTGATEWAY_MCP_ROUTER_URL" router-url})

(defn- call [h method path]
  (let [req (js/Request. (str "https://open-swift.etzhayyim.com" path) #js {:method method})]
         ...
         (-> (js/Promise.all
             ...
             (.then
              (fn [[page health bad pre nf mna dodaf]]
                ...
                ;; 移植しなかったものをページが名指ししている（黙って消していない）
                (check! "page names what was not carried over" true
                        (str/includes? (:body page) "worker/src/app.ts"))
                ;; 移行前の雛形の文言が残っていない
                (check! "the SvelteKit scaffold text is gone" false
                ...
                ;; 入った」——別の主張なので別の検査にする（S6 の M3 で実測）。
                ...
                (check! "wrong method names Allow" "GET" (:allow mna))
                ;; 未 deploy だった app.ts の面を復活させていない
                (check! "undeployed app.ts surface stays 404" 404 (:status dodaf))
                (let [f @failures]
                  (if (seq f)
